...
UI SFX is a tiny semantic sound system for web apps, mobile apps, SaaS, education, media, and games. Call `success`, `drop`, or `level-up` once, then switch the whole product between ten coherent sonic personalities without changing interaction code.

- 78 semantic cues across 13 interaction categories
- 10 complete sound packs
- 780 original sounds in both MP3 and Ogg
- 72 brief one-shots and 6 seamless state loops
- 4.01 MB for every MP3 or 1.95 MB for every Ogg
- 8.8 kB compressed Web Audio runtime with zero dependencies
...
### ElevenLabs research pipeline

The repository includes a guarded pipeline for evaluating alternate timbres:

```bash
ELEVENLABS_API_KEY=... npm run generate:elevenlabs -- --pack organic
```

Provider output is staged outside the release library until redistribution rights and provenance are verified. Research files are ignored and never enter the npm package.
...
Issues and focused pull requests are welcome. Read [CONTRIBUTING.md](CONTRIBUTING.md) before changing the taxonomy or synthesis recipes; a new cue must work in all ten packs and keep a visible or haptic counterpart.

## License

The TypeScript code is [MIT licensed](LICENSE). The procedurally generated audio library is dedicated to the public domain under [CC0 1.0](LICENSE-AUDIO), as are the [category illustrations](LICENSE-ART). Provider-backed evaluation files are not part of the release.
