...
Prioritize meaningful state transitions: confirmed saves, validation outcomes, toggles, tabs, dialogs, completed destructive actions, undo/redo, drag and drop results, uploads, generation, recording, messages, media controls, purchases, milestones, and connection state. Avoid sound for scrolling, every keystroke, passive layout changes, disabled controls, dense-list repetition, background refreshes, and routine hover. Use hover only on sparse important controls with a fine pointer, never touch. Throttle rapid seek, volume, hover, progress, and notification events.
