...
.sponsor-button__heart,
.sponsor-button__arrow {
  display: grid;
  place-items: center;
  flex: 0 0 auto;
}
...
.sponsor-button__heart {
  color: var(--sponsor-shadow);
  transform-origin: 50% 68%;
}

.sponsor-button__signal {
  height: 1rem;
  display: inline-flex;
  align-items: center;
  gap: 0.14rem;
...
.sponsor-button__signal i:nth-child(2) {
  height: 72%;
}
...
@media (hover: hover) and (pointer: fine) {
  .sponsor-button:hover {
    ...
    box-shadow: 0.44rem 0.44rem 0 var(--sponsor-shadow);
  }

  .sponsor-button:hover .sponsor-button__heart {
    animation: sponsor-heart 620ms var(--ease-out-quart) both;
  }

  .sponsor-button:hover .sponsor-button__signal i {
    animation: sponsor-signal 580ms ease-in-out infinite alternate;
  }
...
.sponsor-button:active {
  ...
  box-shadow: 0.08rem 0.08rem 0 var(--sponsor-shadow);
}

@keyframes sponsor-heart {
  0%, 100% { transform: scale(1); }
  42% { transform: scale(1.28) rotate(-6deg); }
  68% { transform: scale(0.94) rotate(3deg); }
}

@keyframes sponsor-signal {
  from { scale: 1 0.55; }
  to { scale: 1 1; }
}
