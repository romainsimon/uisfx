...
const genericPrompt = buildAgentImplementationPrompt()
...
assert.match(genericPrompt, /Choose the sound pack yourself after inspecting the product/)
assert.match(genericPrompt, /Review every available pack in https:\/\/uisfx\.com\/uisfx-catalog\.json/)
assert.match(genericPrompt, /pack: '<selected-pack>'/)
assert.match(genericPrompt, /uisfx\/sounds\/<selected-pack>\/\{cue\}\.mp3/)
assert.match(genericPrompt, /ui\.setPack\('<selected-pack>'\)/)
assert.match(genericPrompt, /agent-guide\.md\?pack=<selected-pack>/)
assert.doesNotMatch(genericPrompt, /pack: 'minimal'/)
...
const genericPromptResponse = await getRoute({ handler: agentPromptRoute, path: '/agent-prompt.txt', contentType: 'text/plain', noindex: true })
assert.equal(await genericPromptResponse.text(), genericPrompt)
